...
from utils import safe_image_loader, safe_downscale, save_checkpoint
...
class SimpleCNN(nn.Module):
    ...
    def __init__(self, num_classes=5, dropout=0.3):
        ...
        self.block1 = nn.Sequential(conv(3, 32), conv(32, 32))
        self.block2 = nn.Sequential(conv(32, 64), conv(64, 64))
        self.block3 = nn.Sequential(conv(64, 128), conv(128, 128))
        self.pool = nn.MaxPool2d(2)
        self.dropout = nn.Dropout(p=dropout)
        self.classifier = nn.Linear(128, num_classes)

    def forward(self, x):
        x = self.block1(x); x = self.pool(x)
        x = self.block2(x); x = self.pool(x)
...
if __name__ == "__main__":
    ...
    # Class-balanced sampling (inverse frequency weights)
